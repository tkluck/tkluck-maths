...
function find_standard_duality(W, vars...)
    ...
    for deg_d17 = 1:(D-1)
        ...
        for deg_d15 = deg_d17:(D-1)
            deg_d26 = D - deg_d15
            for deg_d16 = 1:div(D-1, 2)
                deg_d25 = D - deg_d16

                loops += 1

                c = formal_coefficients(typeof(W), :c)
                d17 = generic_quasihomogeneous_polynomial(deg_d17, gradings, c)
                d35 = generic_quasihomogeneous_polynomial(deg_d35, gradings, c)
                d15 = generic_quasihomogeneous_polynomial(deg_d15, gradings, c)
                d26 = generic_quasihomogeneous_polynomial(deg_d26, gradings, c)
                d16 = generic_quasihomogeneous_polynomial(deg_d16, gradings, c)
                d25 = generic_quasihomogeneous_polynomial(deg_d25, gradings, c)
                ...
                if !(1 in eqns || -1 in eqns)
                    ...
                    @show dense_eqns
                    if true || !(rem(one(eltype(dense_eqns)), gröbner_basis(dense_eqns)) == 0)
                        count += 1
                    end
                end
            end
